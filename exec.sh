...
if [ -z ${1} ] || [ ${1} == "-x" ] ; then
	...
	cd ${BOTDIR}
	. venv/bin/activate
	nohup python3 -m userbot >>${UNIXTIME}-darkproject.log 2>&1 & echo "I: Userbot PID is ${!}"
	deactivate
	cd ${RUNDIR}
	exit 0
	...
	echo -e "--daemonize    || -d -- Run in the background, logs will be saved in *-darkproject.log"
	exit 0
else
	echo -e  "DarkProject"
        echo -e  "Licensed under Raphielscape Public License"
